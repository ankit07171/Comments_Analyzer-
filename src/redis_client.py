...
class UpstashRedis:
    ...
    def _call_post(self, path: str, body: str, timeout: int = 15) -> dict | None:
        """Like _call(), but sends `body` as the raw POST body instead of a
        URL segment. Upstash's REST API accepts both, but GET-with-value-in-
        the-URL breaks down for anything bigger than a few KB (URL length
        limits) — so any value that could be large (like a whole analyzed
        dataset) must go through this instead of `_call('set', key, value)`.
        """
        if not self.is_configured():
            return None
        try:
    ...
    # ---------------- basic ops (small values — dedupe keys, counters) ----------------
    ...
    # ---------------- large-value ops (cached analysis results) ----------------

    def set_raw(self, key: str, text: str, ex_seconds: int | None = None) -> bool:
        """Store an arbitrarily large string (e.g. a whole analyzed dataset
        serialized as JSON) via POST, so it isn't limited by URL length."""
        path = f"set/{key}"
        if ex_seconds:
    ...
    def get_raw(self, key: str) -> str | None:
        """Retrieve a value stored with set_raw(). Plain GET is fine here —
        only the *write* path (value inside the URL) breaks on large data,
        reads are unaffected since the key, not the value, is in the URL."""
        return self.get(key)
